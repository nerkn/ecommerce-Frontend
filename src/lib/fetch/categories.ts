import { Category, ProductCategories } from "src/types/site";
import { fetchX } from "../fetchx";

export async function fetchCatIdsByProduct(productId:number):Promise<number[]>{
    return  fetchX('n2n?where=app,eq,product|bin,eq,category|t1,eq,'+productId).
        then(r=>r.map((t:ProductCategories) =>t.t2))
}
