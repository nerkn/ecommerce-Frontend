...
export default function ProductForm({
    ...
    console.log(product)
    return (
      <form className="flex flex-col form gap-4"  onSubmit={formSubmit()} action="/api/v1/product">
        ...
        <textarea name="description" className="w-full h-48" defaultValue={
        product.description.
            replaceAll('&uuml;','ü').   replaceAll('&Uuml;','Ü').
            replaceAll('&ouml;','Ü').   replaceAll('&Ouml;','Ö').
            replaceAll('&ccedil;','ç'). replaceAll('&Ccedil;','Ç')
        } />
        </div>
        <div className="flex">
            <label className="w32trpr4">AI Description</label>
            <div className="p-4 border ">{desc} 
                <a   className="button"  onClick={sorBakalim}>generate</a>
                {loading}
            </div>
        </div>
        <div className="flex">
