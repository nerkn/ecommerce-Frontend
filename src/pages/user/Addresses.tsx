...
export function Addresses({
  ...
  useEffect(() => {
    if (user?.id) getAddresses(user.id)
  }, [user])
  useEffect(() => {
  ...
  return (
    <div className="Addresses">
      <div className="formPlace">
        <h3 className=" mb-4 text-center">{selected.id ? 'Düzenle' : 'Yeni Adres'}</h3>
        <form
          action="/api/v1/useraddress"
          className="form "
          key={selected.id}
        ...
        >
          ...
          <b>Başlık</b>
          <input name="title" value={selected.title} onChange={(e) => onChange4Selected('title', e.target.value)} />
          <b>Şehir</b>
          <input name="city" value={selected.city} onChange={(e) => onChange4Selected('city', e.target.value)} />
          <b>Adres</b>
          <input
            name="address"
          ...
          />
          <b>Detay</b>
          <input name="detail" value={selected.detail} onChange={(e) => onChange4Selected('detail', e.target.value)} />
          <b></b>
          <input type="submit" value="Kaydet" />
        </form>
      </div>
      {addresses.map((a) => (
        <div className={'oneAddress ' + (selected.id == a.id ? 'selected' : '')} key={a.id}>
          <div className="title">{a.title}</div>
          <div className="flex ">
            <div className="city pr-4">
              {a.city}
              <div>
                {selected.id != a.id ? (
                  <a className="button" onClick={() => selectedSet(a)}>
                    Seç
                  </a>
                ) : (
                  ''
      ...
      <div className={selected.id ? 'oneAddress ' : 'hidden'}>
        ...
        <div className="flex ">
          <a className="button" onClick={() => selectedSet({ ...noAddress })}>
            Yeni
          </a>
        </div>
      </div>
